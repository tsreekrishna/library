...
def _load_models(algorithm: str = 'xgb', classifier_type: str = 'mwp', conformal: bool = False):
    ...
    if conformal == False:
        ...
        if classifier_type == 'mw':
            model_path = pkg_resources.resource_filename('crop_classification', 'trained_models/mw_xgb_mapie.pkl')
            model = pickle.load(open(model_path, 'rb'))
        elif classifier_type == 'mwp':    
            model_path = pkg_resources.resource_filename('crop_classification', 'trained_models/mwp_xgb_mapie.pkl')
            model = pickle.load(open(model_path, 'rb'))
    return model
...
def _load_scalers(classifier_type: str = 'mwp'):
    ...
    if classifier_type == 'mw':
        ...
        scaler_path = pkg_resources.resource_filename('crop_classification', 'trained_models/mwp_standard_scaler.pkl')
        scaler = pickle.load(open(scaler_path, 'rb'))
