...
from .data_preprocess import get_model_data, conformal_prediction, _generate_label_set_map
...
classifier_path = pkg_resources.resource_filename('crop_classification', 'src/models/four_crop_classifier.pkl')
classifier = pickle.load(open(classifier_path, 'rb'))

def model_point_predictions(data: list[list[int]]) -> [list[list[int]], list]:
    '''
    ...
    point_pred = classifier.predict(data)
    pred_prob = classifier.predict_proba(data)
    crop_map = {'0':'Mustard', '1':'Wheat', '2':'Potato', '3':'Rabi Rice'}
...
def conformal_predictions(data, alpha=0.05):
    ...
    X_cal, y_cal = val.drop(['last_fortnight', 'crop'], axis=1), val['crop']
    ...
    cp.fit(X_cal, y_cal)
    _, sets = cp.predict(data, alpha)
    crop_map = {'0':'Mustard', '1':'Wheat', '2':'Potato', '3':'Rabi Rice'}
    crop_set_map = _generate_label_set_map(crop_map)
